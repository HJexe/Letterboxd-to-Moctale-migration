        ...
        try {
            ...
            if (!match) {
                ...
                if (DO_RATINGS && film.rating) {
                    ...
                    // Match the UI preview logic exactly
                    const tag = r >= 5.0 ? "perfection" : r >= 3.5 ? "go_for_it" : r >= 2.0 ? "timepass" : "skip";
                    const reviewRes = await fetch("/api/activity/content/" + slug + "/review", {
                        method: 'POST',
                    ...
                    if (!reviewRes.ok) {
                        console.warn("⚠️ Failed to save rating for:", film.name, "- Status:", reviewRes.status);
                    }
                }
                results.success++;
            }
